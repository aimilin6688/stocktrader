...
class TraderTDX(BaseTrader):
    ...
    def login(self, username=None, password=None, broker=None, comm_password=None, nick_name=None, account:Account=None, **kwargs) -> Result:
        from stocktrader.utils.login_helper import LoginTDX
        return LoginTDX(self).login(username=username, password=password, broker=broker, comm_password=comm_password, nick_name=nick_name, account=account, **kwargs)
